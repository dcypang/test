...
class Game {
  ...
  spawnTraffic(clearOf) {
    ...
    // And the rest of the country. Without this every state outside Idaho is a
    // ghost town: streets, shops, petrol stations, and nobody on the road.
    //
    // What matters is how thick the traffic is where the player happens to be,
    // not how many cars exist: anything more than 320 m away is frozen and
  ...
  registerPenalty(reason, amount) {
    ...
    if (!ds) return;
    // Free roam is not scored, and that has to include collisions. Speeding
    // was already exempt, but hitting something was not - so a scrape a mile
    // off the route still docked a rating for a drive you were not on. It went
    // unnoticed while there was nothing out there to hit.
    if (ds.roaming) return;
  ...
  updateDrive(dt) {
    ...
    //
    // The obstacle list is built once for the whole frame and holds only what
    // is close enough to be an obstacle. It used to be rebuilt inside the loop,
    ...
    for (const t of this.traffic) {
      ...
      if (d < 320) {
        ...
        // Counted, because this is the number that says how hard the relief is
        // working. A build that quietly doubles it has jammed the traffic
        // somewhere, whatever its average speed looks like.
        if (t.stuckFor > (d < 90 ? 15 : 5)) {
          t.done = true;
          t.stuckFor = 0;
          this.stuckRecycles = (this.stuckRecycles || 0) + 1;
        }
      }
